                ...
                <div class="space-y-12">
                    ...
                    <div class="border-b border-gray-900/10 pb-12">
                        ...
                        <p class="mt-1 text-sm leading-6 text-gray-600">Những thông tin này có ảnh hưởng đến giá dịch vụ
                            dịch vụ</p>
                        <Location v-if="from?.service_id" :locationData="from"
                    ...
                    <div class="border-b border-gray-900/10 pb-12">
                        ...
                        <p class="mt-1 text-sm leading-6 text-gray-600">Những thông tin này có ảnh hưởng đến giá dịch vụ
                            dịch vụ</p>
                        <Location v-if="to?.service_id" :locationData="to"
...
import { getLocationsForCreateService, getLocationsByServicId, updateLocation } from "@/services/location.service";
import {
    Combobox,
    ComboboxInput,
    ComboboxButton,
...
const getServiceByIdAxios = async (service_id) => {
    ...
    if (data) {
        // console.log(data);
        let tempServiceTypeId = data.service_types_id;
        // console.log(tempServiceTypeId);

        // Push data to form
        service.value.name = data.service_name;
        service.value.description = data.description;

        service.value.weight = data.weight;
        service.value.price = data.price;
        service.value.delivery_min_time = data.delivery_min_time;
        // console.log(data.delivery_min_time);
        // console.log(data.delivery_max_time);
        service.value.delivery_max_time = data.delivery_max_time;

        providers.value.forEach(element => {
            if (element.id === data.provider_id) {
                // console.log(element);
                selectedProvider.value = element;
            }
        });
        // Lấy ServiceType
        await getServiceTypesByProviderIdAxios(data.provider_id);
        serviceTypes.value.forEach(element => {
            if (element.id == tempServiceTypeId) {
                // console.log(element);
                selectedServiceType.value = element;
                changeServiceType(tempServiceTypeId);
            }
        });

        // Lấy địa chỉ của dịch vụ
        getLocationsByServicIdAxios(data.service_id);
    }
...
const updateServiceAxios = async () => {
    ...
    const { data, error } = await updateService(accessToken, service.value, props.service_id);

    if (data) {
        // console.log(data);

        // update location
        updateLocationAxios(from.value, from.value.id);
        updateLocationAxios(to.value, to.value.id);
        toast.success("Đã cập nhật dịch vụ thành công", { timeout: 3000 });
...
const updateLocationAxios = async (location, location_id) => {
    const accessToken = await getAccessTokenSilently();
    const { data, error } = await updateLocation(accessToken, location, location_id);
    const result = ref({});
