...
import { createServiceType } from "@/services/price-list.service";
...
const priceList = ref({
    ...
    basic_price: 0,
    special_price: 0,
    intra_province_price: '',
    inter_provincial_price: '',
    inter_domain_price: '',
});
...
const createPriceListHandle = async (input) => {
    const accessToken = await getAccessTokenSilently();
    // validate
    ...
    if (data) {
        // update location
        toast.success("Thêm Bảng giá thành công", { timeout: 3000 });
...
function submitHandle(event) {
    ...
    // Update service 
    createPriceListHandle(priceList.value);
}
