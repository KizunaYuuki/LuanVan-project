    ...
    <div v-else>
        <div class="mx-auto max-w-5xl bg-white p-16 pb-4 rounded-lg">
            ...
            <div class="flex-auto mb-10">
                <h1 class="text-[#111827] leading-[3rem] font-[600] text-[1.5rem]">Cập nhật Loại dịch vụ</h1>
            </div>
...
import { getServiceTypeById, updateServiceType } from "@/services/service-type.service";
...
const getServiceTypeByIdAxios = async (id) => {
    const accessToken = await getAccessTokenSilently();
    const { data, error } = await getServiceTypeById(accessToken, id);
    if (data) {
        console.log(data);
        // push service type data
        serviceType.value = data;
        // push provider data
        providers.value.forEach(element => {
            if (element.id === data.providers_id) {
...
const getProvidersAxios = async () => {
    ...
    if (data) {
        ...
        getServiceTypeByIdAxios(props.id);
    }
};

const updateServiceTypeAxios = async (input, id) => {
    const accessToken = await getAccessTokenSilently();
    // validate
    if (serviceType.value.name === '' || serviceType.value.break_weight === '' ||
    ...
    const { data, error } = await updateServiceType(accessToken, input, id);
    if (data) {
        // update location
...
function submitHandle(event) {
    ...
    // Update service 
    updateServiceTypeAxios(serviceType.value, props.id);
}
